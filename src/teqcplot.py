...
# The teqcplot.py web site is at http://code.google.com/p/teqcplot/
...
teqcplot version 1.4.2, 7 Sep. 2014.
...
Teqcplot uses teqc's "plot files" in teqc's COMPACT3 format, made with teqc versions after December 1, 2013.
The first official release of this teqc version will be in 2014. 
A development version of the first 2014 release of teqc is at ftp://brick.unavco.org/pub/users/lou/.
...
Time-Parameter Plots. To make plots with time of day on the x axis and the parameter on the y axis.  Not colored.
  ...
  To change the plot from single data points to a connected line, in the teqcplot.py file, change the 'ko' in
  the line mplot = plt.plot (times, yvals, 'ko',  markersize=1.5 ) to 'k-'.  
...
Spaces " " are not allowed in options. Option order should not matter.
                                               ...
                                               Copyright (C) 2014 Stuart K. Wier
...
def read_input_files ():
    ...
    fileext = string.split(filename2, ".")
    file2 = open (elefile)
    # handle the parm file, if any 
    if len(parmfile) > 5:
	    datapathp = os.path.dirname (parmfile)
	    pfilename =   os.path.basename(parmfile)
            if plottype != "Time-parameter plot":
	       print "  Color by value the "+parmtype+" data from "+pfilename
	    pfile = open (parmfile)
        ...
        if ln>1 and ln % 2 == 0:
            ...
            if  doParmPlot :
              if pfileline[12:14] == "-1" :
                ...
                parmsvslist = string.split(parmsvs," ") # split on whitespace; makes a List
...
def makePlot ():  
    ...
    plt.show()
    # Note: process pauses here until user clicks on something.
...
SVidList.sort()
print    "  There are "+`len(SVidList)`+" SVs with plottable data, and " \
   +`len(SVpositionList)`+" sets of SV, time, azimuth, and elevation."
print "  The SVs with data of this parameter are: " + ', '.join(SVidList)
